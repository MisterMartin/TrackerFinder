# Form implementation generated from reading ui file 'TrackerFinderMainWindow.ui'
#
# Created by: PyQt6 UI code generator 6.4.2
#
# WARNING: Any manual changes made to this file will be lost when pyuic6 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt6 import QtCore, QtGui, QtWidgets


class Ui_TrackerFinderMainWindow(object):
    def setupUi(self, TrackerFinderMainWindow):
        TrackerFinderMainWindow.setObjectName("TrackerFinderMainWindow")
        TrackerFinderMainWindow.resize(518, 551)
        self.centralwidget = QtWidgets.QWidget(parent=TrackerFinderMainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.verticalLayout = QtWidgets.QVBoxLayout(self.centralwidget)
        self.verticalLayout.setObjectName("verticalLayout")
        self.horizontalLayout_2 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_2.setObjectName("horizontalLayout_2")
        spacerItem = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Minimum)
        self.horizontalLayout_2.addItem(spacerItem)
        self.clockFrame = QtWidgets.QFrame(parent=self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Policy.Minimum, QtWidgets.QSizePolicy.Policy.Minimum)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.clockFrame.sizePolicy().hasHeightForWidth())
        self.clockFrame.setSizePolicy(sizePolicy)
        self.clockFrame.setMinimumSize(QtCore.QSize(0, 0))
        self.clockFrame.setFrameShape(QtWidgets.QFrame.Shape.StyledPanel)
        self.clockFrame.setFrameShadow(QtWidgets.QFrame.Shadow.Raised)
        self.clockFrame.setObjectName("clockFrame")
        self.horizontalLayout_2.addWidget(self.clockFrame)
        spacerItem1 = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Minimum)
        self.horizontalLayout_2.addItem(spacerItem1)
        self.verticalLayout.addLayout(self.horizontalLayout_2)
        self.horizontalLayout = QtWidgets.QHBoxLayout()
        self.horizontalLayout.setObjectName("horizontalLayout")
        self.label_12 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_12.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
        self.label_12.setObjectName("label_12")
        self.horizontalLayout.addWidget(self.label_12)
        self.label_13 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_13.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
        self.label_13.setObjectName("label_13")
        self.horizontalLayout.addWidget(self.label_13)
        self.verticalLayout.addLayout(self.horizontalLayout)
        self.gridLayout = QtWidgets.QGridLayout()
        self.gridLayout.setObjectName("gridLayout")
        self.label_16 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_16.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_16.setObjectName("label_16")
        self.gridLayout.addWidget(self.label_16, 0, 0, 1, 1)
        self.pingDeltaT = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.pingDeltaT.setReadOnly(True)
        self.pingDeltaT.setObjectName("pingDeltaT")
        self.gridLayout.addWidget(self.pingDeltaT, 0, 1, 1, 1)
        self.label_15 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_15.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_15.setObjectName("label_15")
        self.gridLayout.addWidget(self.label_15, 0, 2, 1, 1)
        self.aprsDeltaT = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.aprsDeltaT.setReadOnly(True)
        self.aprsDeltaT.setObjectName("aprsDeltaT")
        self.gridLayout.addWidget(self.aprsDeltaT, 0, 3, 1, 1)
        self.label = QtWidgets.QLabel(parent=self.centralwidget)
        self.label.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label.setObjectName("label")
        self.gridLayout.addWidget(self.label, 1, 0, 1, 1)
        self.pingId = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.pingId.setReadOnly(True)
        self.pingId.setObjectName("pingId")
        self.gridLayout.addWidget(self.pingId, 1, 1, 1, 1)
        self.label_5 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_5.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_5.setObjectName("label_5")
        self.gridLayout.addWidget(self.label_5, 1, 2, 1, 1)
        self.aprsTag = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.aprsTag.setReadOnly(True)
        self.aprsTag.setObjectName("aprsTag")
        self.gridLayout.addWidget(self.aprsTag, 1, 3, 1, 1)
        self.label_3 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_3.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_3.setObjectName("label_3")
        self.gridLayout.addWidget(self.label_3, 2, 0, 1, 1)
        self.pingLat = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.pingLat.setReadOnly(True)
        self.pingLat.setObjectName("pingLat")
        self.gridLayout.addWidget(self.pingLat, 2, 1, 1, 1)
        self.label_6 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_6.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_6.setObjectName("label_6")
        self.gridLayout.addWidget(self.label_6, 2, 2, 1, 1)
        self.aprsLat = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.aprsLat.setReadOnly(True)
        self.aprsLat.setObjectName("aprsLat")
        self.gridLayout.addWidget(self.aprsLat, 2, 3, 1, 1)
        self.label_4 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_4.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_4.setObjectName("label_4")
        self.gridLayout.addWidget(self.label_4, 3, 0, 1, 1)
        self.pingLon = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.pingLon.setReadOnly(True)
        self.pingLon.setObjectName("pingLon")
        self.gridLayout.addWidget(self.pingLon, 3, 1, 1, 1)
        self.label_7 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_7.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_7.setObjectName("label_7")
        self.gridLayout.addWidget(self.label_7, 3, 2, 1, 1)
        self.aprsLon = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.aprsLon.setReadOnly(True)
        self.aprsLon.setObjectName("aprsLon")
        self.gridLayout.addWidget(self.aprsLon, 3, 3, 1, 1)
        self.label_2 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_2.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_2.setObjectName("label_2")
        self.gridLayout.addWidget(self.label_2, 4, 0, 1, 1)
        self.pingAge = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.pingAge.setReadOnly(True)
        self.pingAge.setObjectName("pingAge")
        self.gridLayout.addWidget(self.pingAge, 4, 1, 1, 1)
        self.label_11 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_11.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_11.setObjectName("label_11")
        self.gridLayout.addWidget(self.label_11, 4, 2, 1, 1)
        self.aprsTime = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.aprsTime.setReadOnly(True)
        self.aprsTime.setObjectName("aprsTime")
        self.gridLayout.addWidget(self.aprsTime, 4, 3, 1, 1)
        self.label_14 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_14.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_14.setObjectName("label_14")
        self.gridLayout.addWidget(self.label_14, 5, 0, 1, 1)
        self.pingCount = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.pingCount.setReadOnly(True)
        self.pingCount.setObjectName("pingCount")
        self.gridLayout.addWidget(self.pingCount, 5, 1, 1, 1)
        self.label_8 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_8.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_8.setObjectName("label_8")
        self.gridLayout.addWidget(self.label_8, 5, 2, 1, 1)
        self.aprsCount = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.aprsCount.setReadOnly(True)
        self.aprsCount.setObjectName("aprsCount")
        self.gridLayout.addWidget(self.aprsCount, 5, 3, 1, 1)
        self.label_9 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_9.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_9.setObjectName("label_9")
        self.gridLayout.addWidget(self.label_9, 6, 2, 1, 1)
        self.aprsFailed = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.aprsFailed.setReadOnly(True)
        self.aprsFailed.setObjectName("aprsFailed")
        self.gridLayout.addWidget(self.aprsFailed, 6, 3, 1, 1)
        self.label_10 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_10.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight|QtCore.Qt.AlignmentFlag.AlignTrailing|QtCore.Qt.AlignmentFlag.AlignVCenter)
        self.label_10.setObjectName("label_10")
        self.gridLayout.addWidget(self.label_10, 7, 2, 1, 1)
        self.aprsAlt = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.aprsAlt.setReadOnly(True)
        self.aprsAlt.setObjectName("aprsAlt")
        self.gridLayout.addWidget(self.aprsAlt, 7, 3, 1, 1)
        self.verticalLayout.addLayout(self.gridLayout)
        self.log = QtWidgets.QTextEdit(parent=self.centralwidget)
        self.log.setMinimumSize(QtCore.QSize(500, 0))
        self.log.setObjectName("log")
        self.verticalLayout.addWidget(self.log)
        TrackerFinderMainWindow.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=TrackerFinderMainWindow)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 518, 21))
        self.menubar.setObjectName("menubar")
        TrackerFinderMainWindow.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=TrackerFinderMainWindow)
        self.statusbar.setObjectName("statusbar")
        TrackerFinderMainWindow.setStatusBar(self.statusbar)

        self.retranslateUi(TrackerFinderMainWindow)
        QtCore.QMetaObject.connectSlotsByName(TrackerFinderMainWindow)

    def retranslateUi(self, TrackerFinderMainWindow):
        _translate = QtCore.QCoreApplication.translate
        TrackerFinderMainWindow.setWindowTitle(_translate("TrackerFinderMainWindow", "MainWindow"))
        self.label_12.setText(_translate("TrackerFinderMainWindow", "Ping"))
        self.label_13.setText(_translate("TrackerFinderMainWindow", "APRS"))
        self.label_16.setText(_translate("TrackerFinderMainWindow", "DeltaT"))
        self.label_15.setText(_translate("TrackerFinderMainWindow", "Since Last Msg (secs)"))
        self.label.setText(_translate("TrackerFinderMainWindow", "Ping ID"))
        self.label_5.setText(_translate("TrackerFinderMainWindow", "Msg Type"))
        self.label_3.setText(_translate("TrackerFinderMainWindow", "Lat"))
        self.label_6.setText(_translate("TrackerFinderMainWindow", "Lat"))
        self.label_4.setText(_translate("TrackerFinderMainWindow", "Lon"))
        self.label_7.setText(_translate("TrackerFinderMainWindow", "Lon"))
        self.label_2.setText(_translate("TrackerFinderMainWindow", "Age of Fix (ms)"))
        self.label_11.setText(_translate("TrackerFinderMainWindow", "APRS Encoded Time"))
        self.label_14.setText(_translate("TrackerFinderMainWindow", "Ping Msg Count"))
        self.label_8.setText(_translate("TrackerFinderMainWindow", "APRS Msg Counter"))
        self.label_9.setText(_translate("TrackerFinderMainWindow", "GPS Timeouts"))
        self.label_10.setText(_translate("TrackerFinderMainWindow", "Age of Fix (ms)"))


if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    TrackerFinderMainWindow = QtWidgets.QMainWindow()
    ui = Ui_TrackerFinderMainWindow()
    ui.setupUi(TrackerFinderMainWindow)
    TrackerFinderMainWindow.show()
    sys.exit(app.exec())
